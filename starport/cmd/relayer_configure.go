...
func initChain(
	...
	account, err := ca.GetByName(accountName)
	if err != nil {
		return nil, err
	}
	...
	err = cosmosfaucet.TryRetrieve(cmd.Context(), c.ID, rpcAddr, faucetAddr, accountAddr)
	s.Stop()
	...
	coins, err := c.AccountBalance(cmd.Context())
	if err != nil {
		return nil, err
	}
